           ...
           void *user_data)
{
  ...
  sap_socket_send_data(priv_data.socket, ACC_CHANNELID, payload_length, buffer);
}
...
gboolean agent_initialize()
{
  ...
  do {
    result = sap_agent_initialize(priv_data.agent, ACC_ASPID, SAP_AGENT_ROLE_PROVIDER,
